...
public class GoldenRatio {
    ...
    public GoldenRatio(double startingPoint, GoalFunction goalFunction, boolean lambda, Matrix v, Matrix currentPoint){
        this.goalFunction=goalFunction;
        this.v=new Matrix(v);
        this.currentPoint=new Matrix(currentPoint);
        findUnimodalIntervalLambda(startingPoint,v);
    }
    ...
    private void findUnimodalIntervalLambda(double startingPoint, Matrix v) {
        double h = 1E-6;
        ...
        fm = goalFunction.getLambdaValue(startingPoint,v, currentPoint);
        fl = goalFunction.getLambdaValue(l,v,currentPoint);
        fr = goalFunction.getLambdaValue(r,v, currentPoint);
    ...
    private void findUnimodalInterval(Matrix startingPoint,int j) throws MatrixSizeIncompatibleException {
        ...
        //a=a-startingPoint.getElementAt(j,0);
        b=r.getElementAt(j,0);
        //b=b-startingPoint.getElementAt(j,0);
    ...
    public double startLambda(boolean print) {
        if (print)
            System.out.println("Golden ratio:");
        c = b - k * (b - a);
        d = a + k * (b - a);
        double fa = goalFunction.getLambdaValue(a,v, currentPoint);
        double fb = goalFunction.getLambdaValue(b,v, currentPoint);
        double fc = goalFunction.getLambdaValue(c,v,currentPoint);
        double fd = goalFunction.getLambdaValue(d,v, currentPoint);
        while (Math.abs(b - a) > threshold) {
            if(print){
                System.out.println("a: "+a+" c:"+c+" d:"+d+" b:"+b);
                System.out.println("fa: "+fa+" fc:"+fc+" fd:"+fd+" fb:"+fb);
            ...
            if (fc < fd) {
                ...
                fb = fd;
                fd = fc;
                fc = goalFunction.getLambdaValue(c,v,currentPoint);
            } else {
                a = c;
                c = d;
                d = a + k * (b - a);
                fa = fc;
                fc = fd;
                fd = goalFunction.getLambdaValue(d,v, currentPoint);
            }
        }
        //System.out.println("Golden ration solution = [" + a + " " + b + "]");
        if(print){
            System.out.println("Min for x: "+(a + b) / 2);
            System.out.println("Min= "+goalFunction.getLambdaValue((a + b) / 2,v,currentPoint));
        }
        return (a + b) / 2;
    }
}
