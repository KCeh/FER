...
def resolution(clauses, goal):
    ...
    while True:
        ...
        clauses, setOfSupport = removeTautologies(clauses, setOfSupport)
        selectedClauses = selectClauses(clauses, setOfSupport,resolvedPairs)

        if not selectedClauses: return False

        for pair in selectedClauses:
            clause1, clause2 = pair
            resolvents = resolvePair(clause1, clause2)
            resolvedPairs.add(pair)

            if not resolvents.literals:
                return True

            resolved.add(resolvents)

        if resolved.issubset(setOfSupport):
            return False

        setOfSupport|=resolved
...
def removeTautologies(clauses, setOfSupport):
    ...
    for clause in toRemove:
        ...
        elif clause in setOfSupport:
            newSOS.remove(clause)
...
def resolvePair(firstClause, secondClause):
    ...
    toRemove = set()
    #print "first: ", firstClause
    #print "second: ", secondClause
    newClause = firstClause.literals | secondClause.literals

    for literal1 in firstClause.literals:
        for literal2 in secondClause.literals:
            if literal1.negate() == literal2:
                toRemove.add(literal1)

    for literal in toRemove:
        #print "removing", literal, literal.negate()
        newClause.remove(literal)
        newClause.remove(literal.negate())

    #print "newClause: ", Clause(newClause)
...
def selectClauses(clauses, setOfSupport, resolvedPairs):
    ...
    for clause1 in allClauses:
        for clause2 in setOfSupport:
            if clause1 != clause2:
                if clause1.isResolveableWith(clause2):
                    if (clause1, clause2) not in resolvedPairs:
                        selected.add((clause1,clause2))
...
def testResolution():
    ...
    #primjer iz prez -> krivi cilj
    premise1 = Clause(set([Literal('u', (0, 0), False), Literal('t', (0, 0), True)]))
    premise2 = Clause(set([Literal('t', (0, 0), False), Literal('a', (0, 0), False)]))
    premise3 = Clause(set([Literal('u', (0, 0), True), Literal('a', (0, 0), True)]))

    goal = Clause(set([Literal('t', (0, 0))]))
    print resolution(set([premise1, premise2, premise3]), goal)

    #laksi primjer iz prez
    premise1 = Clause(set([Literal('f', (0, 0), False)]))

    goal = Clause(set([Literal('f', (0, 0)), Literal('g', (0, 0))]))
    print resolution(set([premise1]), goal)

    # laksi primjer iz prez->krivi cilj->false rj
    premise1 = Clause(set([Literal('f', (0, 0), False)]))

    goal = Clause(set([Literal('f', (0, 0), True), Literal('g', (0, 0))]))
    print resolution(set([premise1]), goal)

    #primjer s https://www.cs.rochester.edu/~nelson/courses/csc_173/proplogic/resolution.html
    #false rj
    premise1 = Clause(set([Literal('a', (0, 0), False), Literal('i', (0, 0), True)]))
    premise2 = Clause(set([Literal('w', (0, 0), True), Literal('i', (0, 0), False)]))
    premise3 = Clause(set([Literal('a', (0, 0), True)]))
    premise4 = Clause(set([Literal('w', (0, 0)), Literal('i', (0, 0)), Literal('f', (0, 0))]))

    goal = Clause(set([Literal('f', (0, 0), True)]))
    print resolution(set([premise1, premise2, premise3, premise4]), goal)

    # primjer s https://www.cs.rochester.edu/~nelson/courses/csc_173/proplogic/resolution.html
    # true rj
    premise1 = Clause(set([Literal('r', (0, 0), True), Literal('u', (0, 0))]))
    premise2 = Clause(set([Literal('u', (0, 0), True), Literal('w', (0, 0), True)]))
    premise3 = Clause(set([Literal('r', (0, 0)), Literal('w', (0,0), True)]))

    goal = Clause(set([Literal('w', (0, 0), True)]))
    print resolution(set([premise1, premise2, premise3]), goal)

    # primjer s https://www.cs.rochester.edu/~nelson/courses/csc_173/proplogic/resolution.html
    # true rj
    premise1 = Clause(set([Literal('C', (0, 0), True), Literal('A', (0, 0))]))
    premise2 = Clause(set([Literal('D', (0, 0), True), Literal('A', (0, 0))]))
    premise3 = Clause(set([Literal('C', (0, 0))]))
    premise4 = Clause(set([Literal('E', (0, 0))]))

    goal = Clause(set([Literal('A', (0, 0))]))
    print resolution(set([premise1, premise2, premise3, premise4]), goal)
