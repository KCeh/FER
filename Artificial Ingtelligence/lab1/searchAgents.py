...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                visited=list(state[1]) #da se prekopiraju visited od roditelja
                ...
                cost = 1 #zamjena za definiranje costFn preko lambde
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    if len(notVisitedCorners) == 0 : return 0
    ...
    while len(notVisitedCorners) != 0:
        ...
        for i in range(len(notVisitedCorners)):
            dist=util.manhattanDistance(pos, notVisitedCorners[i])
            if dist<minDist:
                minDist=dist
                closesCornerIndex=i
        pos=notVisitedCorners[closesCornerIndex]
        distance=distance+minDist
        notVisitedCorners.remove(notVisitedCorners[closesCornerIndex])
    return distance
