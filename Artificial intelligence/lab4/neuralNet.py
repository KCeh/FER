...
class NeuralNetwork(object):
	...
	def outputs(self, X):
		...
		outputVector=[]
		for inputInstance in X:
			outputVector.append(self.output(inputInstance))
	...
	def total_error(self, predictions, Y): 
		...
		for i in range(0, len(predictions)):
			totalError += self.error(predictions[i], Y[i])
		totalError =float(totalError) / float(len(predictions))
		return totalError
	...
	def forwardStep(self, X, Y):
		...
		totalError=self.total_error(predictions,Y)
		return totalError
